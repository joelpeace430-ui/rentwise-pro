...
const IncomeExpenseChart = ({ selectedYear }: IncomeExpenseChartProps) => {
  ...
  const growthRate = totalIncome > 0 ? ((totalIncome - taxSummary.totalExpenses) / totalIncome) * 100 : 0;

  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload;
      return (
        <div className="rounded-xl border bg-background/95 p-4 shadow-xl backdrop-blur-sm">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-between gap-8">
              ...
              <span className="font-medium text-foreground">{formatCurrency(data.estimatedTax)}</span>
            </div>
            <div className="mt-2 border-t pt-2">
              <span className={`text-xs font-medium ${data.isPaid ? "text-success" : "text-warning"}`}>
                {data.isPaid ? "✓ Instalment paid" : "⏳ Pending"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription className="mt-1">
              Income and KRA instalment tax for {selectedYear}
            </CardDescription>
          </div>
          <div className="flex items-center gap-2 rounded-xl bg-success/10 px-3 py-1.5">
            {growthRate >= 0 ? (
      ...
      <CardContent className="pt-4">
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                <linearGradient id="colorIncomeKE" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="hsl(var(--success))" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="hsl(var(--success))" stopOpacity={0.05} />
                </linearGradient>
                <linearGradient id="colorTaxKE" x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="mt-4 grid grid-cols-4 gap-3 border-t pt-4">
          {chartData.map((quarter, index) => (
            ...
            >
              ...
              <p className="mt-1 text-lg font-bold text-foreground">{formatCurrency(quarter.income)}</p>
              <p className="text-xs text-muted-foreground">Tax: {formatCurrency(quarter.estimatedTax)}</p>
            </div>
          ))}
        </div>
      </CardContent>
